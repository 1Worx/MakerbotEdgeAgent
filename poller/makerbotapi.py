...
class Makerbot(object):
    ...
    def _send_rpc(self, jsonrpc):
        ...
        if self.debug_jsonrpc:
            ...
            #self.rpc_socket.setblocking(0)
            #response2 = self.rpc_socket.recv(2048)
            #sleep(1)
            ##self.rpc_socket.setblocking(1)
            #self._debug_print('JSONRPC', 'RESPONSE', response2)
